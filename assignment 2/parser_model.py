...
class ParserModel(nn.Module):
    ...
    def forward(self, w):
        ...
        logits = self.relu_1(torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        logits = self.dropout(logits)
        logits = self.relu_2(torch.matmul(logits, self.hidden_to_logits_weight) + self.hidden_to_logits_bias)
        ### END YOUR CODE
        return logits
